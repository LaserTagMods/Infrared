...
const int buttonPin=12;  // use pin 12 for the button pin
int buttonState = 0; // variable used for storing the button status
int IRledPin =  13;    // LED connected to digital pin 13 output to send IR
...
// check button state and perform conditional operation to occur if button is pressed or in "butonState=1"
if (buttonState==1){
...
// slashes on the commands and code sections below and adding them to the unaplicable ones.
 ...
 delay(300); // wait for next respawn signal
 SendAlphaRespawn();  // remove the forward slashes to activate, add forward slashes to deactivate
 delay(300); // wait for next respawn signal
 SendAlphaRespawn();  // remove the forward slashes to activate, add forward slashes to deactivate
